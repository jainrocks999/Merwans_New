...
const CategoryList = () => {
    ...
    if (customer_id) {
      ...
      try {
        ...
        if (response.data.status == true) {
          // console.log('this is categoryr',JSON.stringify(response.data.products))
          setFetching(false);
  ...
  const ExpandableComponent = ({ item, onClickFunction }) => {
    ...
    return (
      <View>
        <View style={[styles.cmn, { marginTop: 5 }]}>
          <TouchableOpacity
            activeOpacity={0.2}
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <ImageBackground
              ...
              {category.subcatname || catname.subcatname ? (
                <View style={styles.row1}>
                  ...
                  {category.subcatname == undefined ||
                    ...
                    <Text style={styles.black}>{` /  ${catname ? catname.subcatname : category.subcatname
                      }`}</Text>
                  )}
                </View>
              ) : (
        ...
        <View style={styles.bot}>{/* <BottomTab /> */}</View>
        {/* <Dialog
